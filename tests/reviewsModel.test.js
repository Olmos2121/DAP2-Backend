...
const createReview =
  reviewsModule.createReview ?? reviewsModule.default?.createReview;
const getReview = reviewsModule.getReview ?? reviewsModule.default?.getReview;
const updateReview =
  reviewsModule.updateReview ?? reviewsModule.default?.updateReview;
const deleteReview =
  reviewsModule.deleteReview ?? reviewsModule.default?.deleteReview;
const filterReviews =
  reviewsModule.filterReviews ?? reviewsModule.default?.filterReviews;

if (
  ![createReview, getReview, updateReview, deleteReview, filterReviews].every(
    Boolean
  )
) {
  throw new Error(
    "No se pudieron importar las funciones desde models/reviewsModel.js (revisá exports)."
  );
}
...
describe("reviewsModel", () => {
  ...
  describe("createReview", () => {
    it("inserta y devuelve la reseña cuando película y usuario existen", async () => {
      ...
      expect(_query).toHaveBeenNthCalledWith(
        1,
  ...
  describe("getReview", () => {
    it("devuelve la reseña enriquecida si existe", async () => {
      ...
      expect(_query).toHaveBeenCalledWith(
        expect.stringMatching(/FROM reviews r/i),
        [1]
      );
  ...
  describe("filterReviews", () => {
    ...
    it("funciona sin filtros (todos undefined)", async () => {
      ...
      // Ahora esperamos el filtro por estado activo
      expect(sql).toMatch(/WHERE\s+r\.estado\s*=\s*TRUE/i);

      // Sin filtros dinámicos → sin parámetros
